...
Distinction is also highly sensitive to the span of control. Figure~\ref{fig:span-control} illustrates this effect in a fixed Depth-2 hierarchy. If we increase the number of subordinates per manager from 2 (Binary) up to 10, the distinction score of the middle managers sharply increases while the root node becomes increasingly penalized.

A wider span of control means there are vastly more leaves at the bottom of the hierarchy that have low structural distinction and high constraint. This drags down the average status ($\bar{s}$) of the entire network, mathematically inflating $\alpha$. Middle managers effectively extract more distinction because they monopolize bridging pathways over a larger, more heavily constrained base of subordinates.
