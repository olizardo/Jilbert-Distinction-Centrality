...
Note also that because $\kappa_i = 0$ and $s_1 = 1$, in terms of equation~\ref{eq:distinction} the star node's distinction score thus reduces to:

\begin{equation}
    \beta_1 = \alpha s_1 = \frac{N-1}{1 + \sqrt{N-1}}
    \label{eq:distinction-star}
\end{equation}

Which in the example in Figure~\ref{fig:star} is $7-1 \div (1 + \sqrt{7-1}) = 6 \div (1 + 2.45) = 1.74$ (see Table~\ref{tab:star}). This means that the distinction of the central node in a star graph of a given order increases with the number of nodes (as given by the numerator of equation~\ref{eq:distinction-star}) but at a decreasing rate, as given by the denominator of the same equation. Note also that if we were to forgo scaling and set $\alpha = 1$ in Equation~\ref{eq:distinction}, then distinction reduces to status ($\beta_1 = s_1 = 1.0$) and therefore the distinction of the star node fails to scale with increasing network size (see Figure~\ref{fig:star-scaling}). The hub of a star graph thus has the maximum possible distance between status and constraint. However, with additional spokes added to a star graph, the scalar-adjusted distinction measure increases. The hub of a star graph with seven spokes is more distinct than a hub with six, allowing for a degree of comparability across networks.\footnote{Nodes in a graph composed of a single dyad would have a scalar-adjusted distinction of 0.00 because the average constraint is 0.} 

\subsubsection{Distinction of the Peripheral Nodes}

Similarly, for every peripheral node $j$, the constraint is constant and equal to the status score of the central node $s_1 = 1.0$. Thus, for the peripheral nodes in a star graph, Equation~\ref{eq:distinction} becomes:
...
The path graph is the structure that results when we remove  a single node from a single graph. It thus represents the emergence of status differentiation from where there was none as a result of the removal of a single actor. Figure~\ref{fig:path} shows the path graph of order 7, and ~\ref{tab:path} shows the relevant distinction scores. The pendant nodes (1 and 7), which were previously neighbors of the removed node in the previous circle graph, are now the least distinct. While in the previous circle graph there was no distinction, now we can see that two nodes (2 and 6) emerges as the most distinct, because they bridge between the two pendant nodes and the nodes in the middle of the path; they thus experience the most status and the least constraint, because their exit has very little bearing on the status order given their connection to already status peripheral alters. Interestingly, node four, the most central according to the usual Eigenvector centrality, is also highly constrained, which pulls its distinction score downward. 
...
To explore the behavior of distinction centrality within strict hierarchical structures, we compute the metric across several balanced, undirected rooted trees. These structures classically model organizational hierarchies \citep{friedell1967organizations-81a, krackhardt2014graph}, varying in depth (number of management layers) and branching factor (span of control). 
...
Table \ref{tab:tree-results} displays the computational results for a Depth 2 binary tree (order 7). The global scalar adjustment factor ($\alpha = \bar{\kappa} / \bar{s}$) for this specific network is approximately $0.7714$.
...
Distinction is also highly sensitive to the span of control. Figure~\ref{fig:span-control} illustrates this effect in a fixed Depth-2 hierarchy as we increase the number of subordinates per manager from 2 (Binary) up to 10. For narrow spans of control (e.g., $k=2$ or $3$), intermediaries hold greater distinction because they share a similar baseline status to the root but experience far less constraint. However, as the span of control widens, intermediaries are increasingly weighed down by the massive number of low-status leaves (subordinates) they must connect to, which drastically degrades their eigenvector status ($s_i$). Meanwhile, the root node is connected exclusively to intermediaries, allowing it to maintain a maximum status of $1.0$.
...
\subsubsection{The Special Case of the Pure Autocrat (Star Graph Exception)}
