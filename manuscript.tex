...
We can derive a simplified, exact mathematical expression for the scaled distinction of the root node in any balanced hierarchical tree of depth $\ge 2$. Because the definition of scaled distinction is $\beta_i = \alpha s_i - \kappa_i$, we can evaluate the terms for the root node, whose status $s_i = 1.0$), because the root is the most central node, its initial eigenvector status is exactly $1.0$. The root node's constraint is also $\kappa_i = 1.0$, because when the root node is deleted, the tree shatters into $k$ identical isolated sub-trees. The root's former neighbors (the immediate intermediaries
) become the new ``roots'' of those sub-trees. Because they are the most central nodes in their respective new components, their status in the residual network maxes out at $1.0$. Since all of the root's former neighbors achieve a maximum status, the average constraint the root faces is exactly $1.0$.
