...
As shown in Figure \ref{fig:tree-comparisons}, increasing the depth of the hierarchy shifts who holds the structural leverage. In a deep hierarchy (e.g., Depth 3 or Depth 4), distinction is strictly reserved for the layer of management \textit{immediately above the absolute bottom}. When we add a third or fourth layer (see Figure~\ref{fig:tree-comparisons}), the constraint on the top-level senior managers skyrockets. This occurs because deleting a high-level manager no longer isolates their subordinates. Instead, the remaining subordinates form cohesive sub-components. Because the deleted component retains strong local structure, it retains high status, meaning the high-level manager is heavily constrained by the fact that their subordinates can function cohesively without them.

\subsubsection{The Span of Control (Branching Factor)}
Distinction is also highly sensitive to the span of control. Figure~\ref{fig:span-control} illustrates this effect in a fixed Depth-2 hierarchy as we increase the number of subordinates per manager from 2 (Binary) up to 10. 

For narrow spans of control (e.g., $k=2$ or $3$), middle managers hold greater distinction because they share a similar baseline status to the root but experience far less constraint. However, as the span of control widens, middle managers are increasingly weighed down by the massive number of low-status leaves (subordinates) they must connect to, which drastically degrades their eigenvector status ($s_i$). Meanwhile, the root node is connected exclusively to middle managers, allowing it to maintain a maximum status of $1.0$.

A wider span of control means there are vastly more leaves at the bottom of the hierarchy that have low structural distinction. This drags down the average status ($\bar{s}$) of the entire network, mathematically inflating $\alpha$. Because the root maintains maximum status, its distinction score ($\beta_{root} = \alpha - 1$) grows linearly with $\alpha$, eventually overtaking the middle managers (at $k=7$). This mathematically formalizes a classic organizational dynamic: while middle management holds structural leverage in narrow, deep hierarchies, a sufficiently wide span of control restores supreme distinction to the autocrat at the top by structurally impoverishing the managers below them.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\linewidth]{Plots/span_of_control_distinction.png}
    \caption{The evolution of scaled distinction ($\beta_i$) for the root node and middle managers as a function of the span of control (branching factor) in a Depth-2 hierarchy.}
...
However, as we saw earlier, \textbf{this rule only applies if the tree has middle management (Depth $\ge 2$)}. If the hierarchy is perfectly flat—a pure Star Graph (Depth 1)—the constraint of the root drops to exactly zero. Deleting the root of a star graph isolates every single alter, dropping their eigenvector status to 0. Thus, the root of a star graph achieves maximum positive distinction ($\beta_{root} = \alpha(1) - 0 = \alpha$). This elegantly formalizes the classic dilemma of bureaucratic control: the only way to be a truly unconstrained autocrat is to maintain a flat organization where everyone reports directly to the center. But as scale forces the addition of middle management, the center becomes structurally constrained ($\beta_{root} = \alpha - 1$), and structural leverage immediately transfers to the managers directly controlling the lowest levels.
