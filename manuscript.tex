...
Which in the example in Figure~\ref{fig:star} is $7-1 \div (1 + \sqrt{7-1}) = 6 \div (1 + 2.45) = 1.74$ (see Table~\ref{tab:star}). This means that the distinction of the central node in a star graph of a given order increases with the number of nodes (as given by the numerator of equation~\ref{eq:distinction-star}) but at a decreasing rate, as given by the denominator of the same equation. Note also that if we were to forgo scaling and set $\alpha = 1$ in Equation~\ref{eq:distinction}, then distinction reduces to status ($\beta_{star} = s_{star} = 1.0$) and therefore the distinction of the star node fails to scale with increasing network size (see Figure~\ref{fig:star-scaling}). The hub of a star graph thus has the maximum possible distance between status and constraint. However, with additional spokes added to a star graph, the scalar-adjusted distinction measure increases. The hub of a star graph with seven spokes is more distinct than a hub with six, allowing for a degree of comparability across networks.\footnote{Nodes in a graph composed of a single dyad would have a scalar-adjusted distinction of 0.00 because the average constraint is 0.} 
...
This tells us that the distinction of the peripheral node in the star graph will always be negative (given the numerator) and will decrease steadily towards zero (at a decreasing rate) as the order of the graph increases, as given by the denominator (see Figure~\ref{fig:star-scaling}). For the example in Figure~\ref{fig:star}, Equation~\ref{eq:distinction-peripheral2} resolves itself to: $-1 \div (1 + \sqrt{6}) = -1 \div 3.45 = -0.29)$ (see Table~\ref{tab:star}). The sociological logic being that it is of greater distinction to be one of many peripheral nodes than one of few peripheral nodes. The distance between the top and bottom in the system grows towards infinity, but the absolute social meaning in comparison to another system improves. Being essentially undifferentiated in one system is preferable to being on the negative end of stratification in another. 
...
All the nodes in the circle graph (or any other regular lattice of a given degree \textit{k}, as the circle graph is just the special case where $k = 2$) are in a very different situation compared to the central node in the star graph: While they experience the theoretical maximum of status ($s_i = 1.0$), at the same time they experience a great deal of constraint. In the circle graph, the constraint is determined by the average status of nodes in the node-deleted path graph that results from removing any given node from the circle graph, while in the clique graph the constraint equitably experienced by all nodes is the theoretical maximum ($k_i = 1.0$).

The reason why the nodes in the circle graph experience a non-zero level of constraint, is that they are in a situation in which they all contribute equally to the original non-differentiated status order; this means that the \textit{removal} of any one node from the graph will have a \textit{disruptive} effect on such a minimal order, creating a large amount of status differentiation among the remaining where there previously was none.\footnote{Thus, the distinction measure captures the intuition, common to members of solidary small groups, who experience large constraints on exit, precisely due to the consequences that such an exit will have for the others that are left behind \citep{durkheim1933division-cfd, breiger1998solidarity-afa}.} Yet, all nodes in the circle graph experience the \textit{same} amount of constraint, because deleting any node results in the same structure: a path graph of order $N-1$, where $N$ is the number of nodes in the original circle graph. 
...
In the clique graph, nodes have the same constant status scores as in the circle graph, set at maximum ($s_i = 1.0$) but they also experience maximum constraint ($\kappa_i = 1.0$), as shown in Table~\ref{tab:clique}. The reason for that is that removing a node from the clique graph has no effect on the status of its previous neighbors, which remains equal to 1.0 in the resulting node-deleted clique of order $N-1$. For that reason, since both the both terms of the subtraction in the right-hand side of~\ref{eq:distinction} are equal to 1.0, the resulting distinction scores for all nodes are zero $\beta_i = 0$.\footnote{Because of the local nature of the $\kappa$ component in Equation~\ref{eq:distinction}, in a regular $(n, k)$ graph of order $n$ with prescribed degree $k$ across nodes, we should not expect scaled distinction to be exactly zero, since the focal nodes previous neighbors will have different status scores than the other nodes in the graph in the focal-node-deleted subgraph. However, we should not expect their status to be wildly different, since their degree is now only $k-1$. Instead, we should expect scaled distinction to fluctuate \textit{around} the zero value across nodes. Nevertheless, these should cancel out across nodes; therefore, we should also expect the \textit{average} distinction across nodes in a regular $(n, k)$ graph to be zero.} 
...
The star graph represents an extreme, if overly simplified conception of hierarchy and inequality \citep{freeman79}. Rooted tree graphs, provide a structure with more verisimilitude to real world hierarchical systems. To explore the behavior of distinction centrality measure within such hierarchical structures, we compute the metric across several balanced, undirected rooted trees. These structures classically model organizational hierarchies \citep{friedell1967organizations-81a, krackhardt2014graph}, varying in depth (number of intermediary layers) and branching factor (span of control). 
...
\subsubsection{The Leverage of Middle Management}
...
\subsubsection{The Special Case of the Pure Autocrat}
