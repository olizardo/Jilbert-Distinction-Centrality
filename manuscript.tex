...
To explore the behavior of distinction centrality within strict hierarchical structures, we compute the metric across several balanced, undirected rooted trees. These structures classically model organizational hierarchies \citep{krackhardt1988informal-42d, friedell1967organizations-81a} varying in depth (number of management layers) and branching factor (span of control). 

\begin{figure}[ht!]
    \centering
    \includegraphics[width=\linewidth]{Plots/tree_comparisons.png}
    \caption{A comparison of distinction centrality ($\beta_i$) across three different hierarchical structures: a Star Graph (Depth 1), a binary tree of Depth 2, and a binary tree of Depth 3. Nodes are colored by their scaled distinction scores.}
    \label{fig:tree-comparisons}
\end{figure}
...
\subsection*{The Effect of Hierarchical Depth}
As shown in Figure \ref{fig:tree-comparisons}, increasing the depth of the hierarchy shifts who holds the structural leverage. In a deep hierarchy (Depth 3; order 15), distinction is strictly reserved for the layer of management \textit{immediately above the absolute bottom}. 

When we add a third layer, the constraint on the top-level senior managers skyrockets. This occurs because deleting a high-level manager no longer isolates their subordinates. Instead, the remaining subordinates form cohesive sub-components (effectively acting as star graphs around the lower-level managers). Because the deleted component retains strong local structure, it retains high status, meaning the high-level manager is heavily constrained by the fact that their subordinates can function cohesively without them.
