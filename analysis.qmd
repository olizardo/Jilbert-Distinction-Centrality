...
```{r Toy Examples}
   ...
   g8 <- make_full_graph(6, directed = FALSE)
   g8 <- add_vertices(g7, 1)
   g8 <- g8 + edge(1,7) # clique plus pendant
   g9 <- make_full_graph(7, directed = FALSE) # clique
```
...
```{r Toy Networks Plots}
   ...
   dev.off()
   png(here("Plots", "circle-plus-triangle.png"))
   plot.graph.norm(g6, l = "circle")
   dev.off()
   png(here("Plots", "circle-plus-long.png"))
   plot.graph.norm(g7, l = "circle")
   dev.off()
   png(here("Plots", "kite.png"))
   plot.graph.norm(g8, l = "kk")
   dev.off()
   png(here("Plots", "clique.png"))
   plot.graph.norm(g9, l = "circle")
   dev.off()
```
...
```{r}
   ...
   wrap_plots(p.list[c(1, 6, 11, 16)], nrow = 2)
   ggsave(here("Plots", "connected-caves-examples.png"), width = 10, height = 10, units = "in")
```
