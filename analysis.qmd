...
```{r Mediator versus Weld}
   ...
   c1 <- c4; c2 <- c4
   V(c1)$name <- as.character(1:4); V(c2)$name <- as.character(4:7)
   sf4 <- del.names(c1 + c2)
   process_toy(sf4, "sf4.png", "sf4", "Distinction centrality scores for the structural fold graph (4-Clique).")
   
   ## Structural Fold (5-Clique)
   c1 <- c5; c2 <- c5
   V(c1)$name <- as.character(1:5); V(c2)$name <- as.character(5:9)
   sf5 <- del.names(c1 + c2)
   process_toy(sf5, "sf5.png", "sf5", "Distinction centrality scores for the structural fold graph (5-Clique).")
   
   ## Triangle Broker
   b3 <- add_vertices(c3 + c3, 1) + edge(7,1) + edge(7,4)
   process_toy(b3, "broker3.png", "b3", "Distinction centrality scores for broker graph (3-clique).")
   
   ## Clique Broker
   b4 <- add_vertices(c4 + c4, 1) + edge(9,1) + edge(9,5)
   process_toy(b4, "broker4.png", "b4", "Distinction centrality scores for broker graph (4-Clique).")
```
...
![Star Graph Roles by Size](Plots/star-roles-by-size.png)

## Connected Caves Computational Experiments

![Connected Caves 1](Plots/connected-caves-examples1.png)
![Connected Caves 2](Plots/connected-caves-examples2.png)
![Connected Caves 3](Plots/connected-caves-examples3.png)
![Connected Caves 4](Plots/connected-caves-examples4.png)

## Broker Computational Experiments

![Tree Examples](Plots/tree-examples.png)

![Broker Tree Roles](Plots/broker-tree-roles-by-nbranches.png)

![Clique Examples](Plots/clique-examples.png)

![Broker Clique Roles](Plots/broker-cliques-roles-by-nbranches.png)
