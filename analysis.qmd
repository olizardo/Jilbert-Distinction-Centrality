...
```{r}
   ...
   dat <- data.frame(d = c(ds.s, ds.p, d.s, d.p), 
         ...
         mutate(g = factor(g, labels = c(
                           "Scaled Distinction (Star)", 
                           "Scaled Distinction (Peripheral)", 
                           "Non-scaled Distinction (Star)",
                           "Non-scaled Distinction (Peripheral)"))) |> 
         mutate(n = n + 2)
   ...
   p
   ggsave(here("Plots", "star-roles-by-size.png"), width = 7, height = 7, units = "in")
```
