...
# Calculate distinction scores to find global limits
res1 <- distinction(g1, norm = "abm")$scd
res2 <- distinction(g2, norm = "abm")$scd
res3 <- distinction(g3, norm = "abm")$scd

global_min <- min(c(res1, res2, res3))
global_max <- max(c(res1, res2, res3))

# Helper function to plot
plot_network_snap <- function(g, res_scd, title) {
  ...
  ggraph(g, layout = layout_coords) +
    ...
    geom_node_point(aes(fill = res_scd), shape = 21, size = 6, color = "black") +
    scale_fill_gradient2(low = "dodgerblue", mid = "gray90", high = "firebrick", 
                         name = "Scaled\nDist.", limits = c(global_min, global_max)) +
    labs(title = title) +
    theme_void() +
    theme(
...
p1 <- plot_network_snap(g1, res1, "1. Initial Lattice (Density: 0.07)")
p2 <- plot_network_snap(g2, res2, "2. Small World (Density: ~0.13)")
p3 <- plot_network_snap(g3, res3, "3. Dense Random (Density: ~0.53)")
