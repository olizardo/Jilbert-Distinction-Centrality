distinction <- function(x, norm = "max", digits  = 4) { 
  ...
  iso <- sum(components(g)$csize == 1) #checking for isolated components
  if (nc == 1) {s <- eig.x$vectors[, 1]} #collecting status scores from first eigenvector in connected graph
  if (nc > 1 & iso > 0) {s <- eig.x$vectors[, 1]} #collecting status scores from first eigenvector in connected graph
  if (nc > 1 & iso == 0) {s <- rowSums(eig.x$vectors[, 1:nc])} #collecting status scores from separate eigenvectors in disconnected graph
  ...
  for (i in 1:n) { #start looping across nodes in the graph
    ...
    if (ncd == 1) {sd <- eig.xd$vectors[, 1]} #status scores for connected node-deleted subgraph
    if (ncd > 1 & iso == 0) {sd <- rowSums(eig.xd$vectors[, 1:ncd])} #status scores for disconnected node-deleted subgraph
    if (ncd > 1 & iso > 0) {sd <- eig.xd$vectors[, 1]} #status scores for disconnected node-deleted subgraph with isolated components
